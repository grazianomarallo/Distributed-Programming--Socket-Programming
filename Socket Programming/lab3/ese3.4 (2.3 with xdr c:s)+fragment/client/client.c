 ...
 * XDR mode: receive file length before file
    ...
    while(1) {
        ...
        if(m.tag == OK) {
            ...
            while(length < tot) {
                ...
                if(fwrite(m.message_u.fdata.contents.contents_val, sizeof(char), m.message_u.fdata.contents.contents_len, f) != m.message_u.fdata.contents.contents_len) {
                    printf("Error during file write\n");
                    break;
                }
            }
