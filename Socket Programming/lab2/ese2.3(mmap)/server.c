...
int proto_get(char *filename, int s) {
    ...
    if(writen(s, sbuf, 13) != 13) {
        printf("Write error while replying\n");
        return 0;
    }
    
    file = mmap (0, length, PROT_READ,  MAP_PRIVATE, fd, 0);
    map = file;
    
    for(n = 0; n < length / FBUFLEN; n++) {
        if(writen(s, file, FBUFLEN) != FBUFLEN) {
            printf("Write error while replying\n");
            return 0;
        }
        file += FBUFLEN;
    }
    if(writen(s, file, length % FBUFLEN) != length % FBUFLEN) {
        printf("Write error while replying\n");
        return 0;
    }
